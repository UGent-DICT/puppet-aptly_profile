...
define aptly_profile::trusted_key(
  ...
  exec { "aptly_profile::trusted_key import aptly GPG key ${title} (${comment}) in to keyring":
    ...
    command     => "/bin/echo '${key}' | ${facts['gpg_path']} --no-default-keyring --keyring trustedkeys.gpg --import",
    unless      => "${facts['gpg_path']} --no-default-keyring --keyring trustedkeys.gpg --list-keys '${title}'",
